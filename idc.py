...
st.set_page_config(page_title='売り上げ分析（IDC大塚）')
st.markdown('#### 売り上げ分析（IDC大塚）')
...
original_list = ['AS YOU ARE', 'BUNA OAK', 'BUNA WALNUT', 'AWASE', 'HIDA collection', 'KAYAⅡ(大塚ｵﾘｼﾞﾅﾙ)', 'LINSIFA(ﾘｰﾝｿﾌｧｰ)', '森のささやき', '明家']

# オリジナル比率（全体）
def original_ratio():
    now_original_sum = df_now[df_now['シリーズ名'].isin(original_list)]['金額'].sum()
    last_original_sum = df_last[df_last['シリーズ名'].isin(original_list)]['金額'].sum()
    ...
    with col1:
        ...
        st.caption('AS YOU ARE/BUNA OAK/BUNA WALNUT/AWASE/HIDA collection/KAYAⅡ(大塚ｵﾘｼﾞﾅﾙ)/LINSIFA(ﾘｰﾝｿﾌｧｰ)/森のささやき/明家')
    ...
    for customer in customer_list:
        ...
        original_now_culc = df_now_cust[df_now_cust['シリーズ名'].isin(original_list)]['金額'].sum()
        original_last_culc = df_last_cust[df_last_cust['シリーズ名'].isin(original_list)]['金額'].sum()
...
def original_ratio_d():
    ...
    #ダイニング　オリジナル売上
    now_original_d_sum = df_now_d[df_now_d['シリーズ名'].isin(original_list)]['金額'].sum()
    last_original_d_sum = df_last_d[df_last_d['シリーズ名'].isin(original_list)]['金額'].sum()
    #ダイニング　オリジナル比率
    ...
    for customer in customer_list:
        ...
        original_now_d_culc = df_now_cust_d[df_now_cust_d['シリーズ名'].isin(original_list)]['金額'].sum()
        original_last_d_culc = df_last_cust_d[df_last_cust_d['シリーズ名'].isin(original_list)]['金額'].sum()
...
def original_ratio_l():
    ...
    #リビング　オリジナル売上
    now_original_l_sum = df_now_l[df_now_l['シリーズ名'].isin(original_list)]['金額'].sum()
    last_original_l_sum = df_last_l[df_last_l['シリーズ名'].isin(original_list)]['金額'].sum()
    ...
    for customer in customer_list:
        ...
        original_now_l_culc = df_now_cust_l[df_now_cust_l['シリーズ名'].isin(original_list)]['金額'].sum()
        original_last_l_culc = df_last_cust_l[df_last_cust_l['シリーズ名'].isin(original_list)]['金額'].sum()
